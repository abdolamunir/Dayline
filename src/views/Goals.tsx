...
export function Goals({ onViewChange, selectedGoalId }: { onViewChange?: (view: string) => void, selectedGoalId?: string }) {
  ...
  const handleRenamePage = () => {
    ...
    const nextTitle = titleEditRef.current?.textContent?.trim() || currentPageTitle;
    if (item && nextTitle) {
      updateSidebarItem(item.id, nextTitle, item.icon);
    }
  ...
  return (
    ...
    >
            ...
            <div className="flex min-w-0 items-center gap-2.5">
              <h1
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'Escape') {
                    e.preventDefault();
                    if (titleEditRef.current) titleEditRef.current.textContent = currentPageTitle;
                    setTitleValue(currentPageTitle);
                    setIsEditingTitle(false);
                  }
                }}
      ...
      <div className="flex flex-col gap-1 flex-1 overflow-hidden">
        ...
        <div
          className={cn(
            "relative flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-[var(--tokyo-border)] pb-2 -mx-4 px-4 md:-mx-8 md:px-8",
            draggingId && "z-[90]"
          )}
        >
