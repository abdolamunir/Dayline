...
export function TaskModal({ task, isOpen, onClose }: TaskModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="bg-[#1C1C1C] rounded-xl w-full max-w-[400px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] text-white/90 font-sans"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3">
          <button className="flex items-center gap-1.5 text-sm font-medium text-white hover:bg-white/5 px-2 py-1 rounded transition-colors">
            Event
            <ChevronDown className="w-4 h-4 text-white/50" />
          </button>
          <div className="flex items-center gap-1 text-white/50">
            <button onClick={handleDelete} className="p-1.5 hover:bg-white/10 hover:text-red-400 rounded transition-colors" title="Delete">
              <MoreHorizontal className="w-4 h-4" />
            </button>
            <button className="p-1.5 hover:bg-white/10 hover:text-white rounded transition-colors">
              <LayoutLeft className="w-4 h-4" />
            </button>
            <button onClick={handleSaveAndClose} className="p-1.5 hover:bg-white/10 hover:text-white rounded transition-colors ml-1">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="overflow-y-auto custom-scrollbar flex-1 pb-4">
          {/* Title */}
          <div className="px-6 py-2 pb-5 border-b border-white/5">
            <input
              type="text"
          ...
          {/* Time & Date Settings */}
          <div className="px-6 py-4 space-y-5 border-b border-white/5">
            {/* Time Row */}
                  ...
                  {editedTask.startTime ? (
                    <>
                      <input 
                        ...
                        className="bg-transparent text-white/90 outline-none w-[90px] cursor-pointer"
                      />
                      ...
                      <input 
                        ...
                        className="bg-transparent text-white/90 outline-none w-[90px] cursor-pointer"
                      />
                      <span className="text-white/40 ml-1">{calculateDuration()}</span>
                    </>
                  ) : (
                    <span className="text-white/90">All Day</span>
                  )}
                </div>
            ...
            <div className="flex items-center gap-4 pl-9">
              <button 
                className={cn(
                  ...
                  !editedTask.startTime ? "bg-purple-500" : "bg-white/20"
                )}
                onClick={() => {
              ...
              <span className="text-[15px] text-white/90">All-day</span>
            </div>

            {/* Timezone */}
            <div className="flex items-center gap-4">
              <Globe className="w-5 h-5 text-white/40 shrink-0" />
              <span className="text-[15px] text-white/40">GMT+5 <span className="text-white/90">Karachi</span></span>
            </div>

            {/* Recurrence */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Refresh className="w-5 h-5 text-white/40 shrink-0" />
                <span className="text-[15px] text-white/90">Every weekday <span className="text-white/40">Mon - Fri</span></span>
              </div>
          ...
          {/* Meeting Details */}
          <div className="px-6 py-4 space-y-5 border-b border-white/5">
            <div className="flex items-center gap-4 text-white/40 cursor-pointer hover:text-white/60 transition-colors">
          ...
          <div className="px-6 py-4 border-b border-white/5">
            <textarea
              ...
              className="w-full bg-transparent text-[15px] text-white/90 placeholder:text-white/40 outline-none resize-none h-8"
            />
          </div>
          ...
          <div className="px-6 py-4 space-y-5">
            <div className="flex items-center gap-4">
              ...
              <span className="text-[15px] text-white/90">abdolamunir@gmail.com</span>
            </div>
            ...
            <div className="flex items-center gap-12 pl-8 relative">
              <button 
                ...
                className="flex items-center gap-2 text-[15px] text-white/90 hover:text-white transition-colors"
              >
                ...
                {isStatusDropdownOpen && (
                  <>
                    ...
                    <motion.div 
                      ...
                      className="fixed z-[70] bg-[#252525] border border-white/10 rounded-xl shadow-2xl p-1.5 w-40 overflow-hidden"
                      style={{ 
                        top: dropdownPos.y, 
                        left: dropdownPos.x 
                      }}
                        ...
                        {[
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              editedTask.status === option.value ? "bg-white/10 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"
                            )}
                          >
              ...
              <span className="text-[15px] text-white/90 cursor-pointer">Default visibility</span>
            </div>

            <div className="flex items-start gap-4">
              <Bell className="w-5 h-5 text-white/40 shrink-0 mt-0.5" />
              <div className="flex flex-col gap-4">
                <span className="text-[15px] text-white/40">Reminders</span>
                <span className="text-[15px] text-white/90">At start of event</span>
              </div>
            </div>
          </div>
        </div>
